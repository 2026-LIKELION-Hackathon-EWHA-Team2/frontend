...
const BottomGNB = ({ items }) => {
  ...
  const overridePath = useGnbOverrideStore((state) => state.overridePath);

  return (
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-[#FEFEFE] rounded-t-[0.625rem] shadow-[0_-2px_2px_0_rgba(0,0,0,0.05)] flex justify-between items-center pt-3.25 pb-[calc(1.625rem+env(safe-area-inset-bottom))] px-10 z-50 print:hidden">
      {items.map((tab) => {
        const isActive = overridePath
          ? tab.path === overridePath
